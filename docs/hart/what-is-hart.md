...
<p>With OmniBus, the record writes itself. Configure, Calibrate, Loop Test, Record: the steps you already work, except each one captures <strong>As-Found / As-Left</strong> calibration as you go. And it never takes the instrument's word for the current. An onboard 4-20 mA circuit measures the real milliamps on its own, so what lands in the record is what the loop is actually doing, not what the device thinks it is doing. Every session is written to an append-only historian that answers who did what, when, where, and why, and finished calibration certificates come out the other side. It reaches instruments over USB HART, an on-board two-wire loop, HART-IP, and wireless, all driving the same workflow.</p>
<p>One technician, one handheld: a vendor-neutral HART communicator and a defensible calibration record, with the paperwork already done. For definitions of any term used here, see the <a href="/hart/glossary/">HART glossary</a>.</p>
