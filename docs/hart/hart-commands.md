...
<p>OmniBus works straight from these standardized tiers, so the screen shows instrument behavior, not raw command numbers or byte payloads. It does not ask which vendor you are holding. It speaks universal and common-practice HART to all of them, and presents the work the way a technician actually thinks about it: range it, trim it, verify it, prove it.</p>
<p>That is what makes it manufacturer-independent: no per-vendor handheld, no descriptor licensing. Commands are sorted by device family (Temperature, Level, Pressure, PID Control), so the screen only ever shows what the instrument in front of you can do. Underneath, one command logic layer drives what the operator sees, what the backend executes, and what the historian records, so behavior is identical on every device class.</p>
<p>It also settles the command 2 caveat instead of inheriting it. A HART device only reports the milliamps it thinks it is putting out, so OmniBus measures the real loop current with its own onboard circuit and writes that trusted number into the As-Found / As-Left record. On the network side, the multi-modal HART-IP engine works out whether it is talking to a wireless gateway or a single instrument and reshapes the screen to match, all on the same command vocabulary underneath.</p>
