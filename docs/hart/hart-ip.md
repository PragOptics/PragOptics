...
<p><strong>OmniBus reaches both HART-IP topologies with one multi-modal engine, and it works out what it is talking to before it draws a screen: a gateway host, or a single instrument.</strong> You hand it the address. It sets up its own side of the network (port and subnet), polls the endpoint, and decides what answered. You configure nothing else.</p>
...
<p>Live values refresh on a steady round-robin, only for what is on screen, and the heartbeat flips to offline the instant the gateway goes quiet. No stale number is ever dressed up as live. Because a gateway is not one instrument, it is a whole network of them, and OmniBus reads the whole mesh and walks into any device on it from one screen. HART-IP over Ethernet is just one way in. USB HART, the on-board two-wire loop, Wi-Fi, Bluetooth: same engine, same audit trail.</p>
