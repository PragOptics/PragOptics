...
export function billingHtml() {
  ...
  let inner;
  if (b.subs == null) inner = `<div class="acct-actions-row"><button class="btn btn-sm" type="button" data-lic-action="bill-read" ${b.busy ? 'disabled' : ''}>${b.busy ? 'Reading Stripe…' : 'Check the licensing charges'}</button></div>`;
    ...
    <div class="acct-actions-row"><button class="btn btn-sm" type="button" data-lic-action="bill-read" ${b.busy ? 'disabled' : ''}>${b.busy ? 'Reading Stripe…' : 'Read again'}</button></div>`;
  return cardHtml({
...
function itemHtml(s, i, e, canceled) {
  ...
  const stop = !owned && !canceled ? `<button class="btn btn-sm is-danger" type="button" data-lic-action="bill-stop" data-sub="${e(s.id)}" data-item="${e(i.id)}" ${b.stopping ? 'disabled' : ''}>${b.stopping === i.id ? 'Stopping…' : 'Stop this charge'}</button>` : '';
  return `
...
async function stopCharge(btn) {
  ...
  if (!btn.dataset.sure) { btn.dataset.sure = '1'; btn.textContent = 'Yes, stop it now'; return; }
