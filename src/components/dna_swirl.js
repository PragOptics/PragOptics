export function mountDnaSwirl(target, opts = {}) {
  ...
  const cfg = {
    ...
    logoUrlPurple: opts.logoUrlPurple ?? 'https://pragoptics.com/images/logo-purple.png',
    logoSize: opts.logoSize ?? null,
    logoFadeInSecs: opts.logoFadeInSecs ?? 0.55,
    logoHoldAlpha: opts.logoHoldAlpha ?? 1.0,
    
    // always-on softness (keep logo organic at full hold)
    logoSoftAlpha: opts.logoSoftAlpha ?? 0.78,

    // idle crossfade speed (seconds per full cycle)
    logoCycleSecs: opts.logoCycleSecs ?? 5.0,

    //  cap how much purple is allowed to dominate in idle (0..1) (0.20 for tint shimmer)
    logoPurpleMaxMix: opts.logoPurpleMaxMix ?? 0.35,

    logoAquaFadeFactor: opts.logoAquaFadeFactor ?? 0.35,

    
    // idle pulse mode: 'sine' | 'heartbeat'
    logoPulseMode: opts.logoPulseMode ?? 'heartbeat',

    // heartbeat tempo (beats per minute). One cycle = one heartbeat pair (lub+dub)
    logoPulseBpm: opts.logoPulseBpm ?? 12,

    // shape tuning (seconds within one cycle)
    logoBeatLubPos: opts.logoBeatLubPos ?? 0.22,
    logoBeatDubPos: opts.logoBeatDubPos ?? 0.77,
    logoBeatWidth:  opts.logoBeatWidth  ?? 0.750,
  ...
  const logoImgPurple = new Image();
  logoImg.crossOrigin = 'anonymous';
  logoImgPurple.crossOrigin = 'anonymous';
  let logoReady = false;
  let logoReadyPurple = false;
  let logoAlpha = 0;
  let logoAlphaPurple = 0;
  
  let idleStartTs = 0;
  let logoMix = 0; // 0..1 (used to preserve mix through resizes)

  logoImg.onload = () => { logoReady = true; };
  logoImg.onerror = () => { logoReady = false; };
  
  logoImgPurple.onload = () => { logoReadyPurple = true; };
  logoImgPurple.onerror = () => { logoReadyPurple = false; };

  logoImg.src = cfg.logoUrl;
  logoImgPurple.src = cfg.logoUrlPurple;
  ...
  function rebuildSlotsForDensity(preserve = true) {
    ...
    if (preserve && (prevPhase === 'idle' || prevPhase === 'logo')) {
      ...
      const base = cfg.logoHoldAlpha * cfg.logoSoftAlpha;
      logoAlpha = base * (1 - logoMix);
      logoAlphaPurple = base * (logoMix);
    } else {
      phase = 'build';
      buildAcc = 0;
      shedAcc = 0;
  ...
  const aA = Math.max(0, Math.min(1, logoAlpha));
  const aB = Math.max(0, Math.min(1, logoAlphaPurple));

  if ((!logoReady || aA <= 0) && (!logoReadyPurple || aB <= 0)) return;
  ...
  if (logoReadyPurple && aB > 0) {
    ctx.globalAlpha = aB;
    ctx.drawImage(logoImgPurple, r.x, r.y, r.w, r.h);
  }
  ...
  function step(now) {
    ...
    if (phase === 'logo') {
      ...
      logoAlpha = p * cfg.logoHoldAlpha * cfg.logoSoftAlpha;
      logoAlphaPurple = 0;
    ...
    if (phase === 'idle') {
      if (!idleStartTs) idleStartTs = now;

      const base = cfg.logoHoldAlpha * cfg.logoSoftAlpha;

      // Convert BPM to cycle seconds.
      // One "cycle" here is one lub+dub pair.
      const cycleSecs = 60 / Math.max(20, cfg.logoPulseBpm);

      const elapsed = (now - idleStartTs) / 1000;
      const frac = (elapsed / cycleSecs) % 1;

      // Heartbeat pulse function 
      const bump = (x, mu, w) => {
        const d = (x - mu) / Math.max(0.01, w);
        return Math.exp(-d * d * 6.0); // sharper than a normal exp(-d^2)
      };

    let k;

    if (cfg.logoPulseMode === 'sine') {
      k = 0.5 - 0.5 * Math.cos(frac * Math.PI * 2);
    } else {
      const lub = bump(frac, cfg.logoBeatLubPos, cfg.logoBeatWidth);
      const dub = bump(frac, cfg.logoBeatDubPos, cfg.logoBeatWidth * 1.15);

      // Combine with slight emphasis on lub 
      k = Math.min(1, (lub * 1.0) + (dub * 0.85));
    }

      const mix = k * cfg.logoPurpleMaxMix;

      logoMix = mix;

      const aquaMul = 1 - (mix * cfg.logoAquaFadeFactor);

      logoAlpha = base * aquaMul;
      logoAlphaPurple = base * mix;
    }
